...
<aside>
⚠️ The output of n-layered attention bridge is `[out, alphas]`, and each type of attention bridge outputs `[alphas, out]`
</aside>
...
Mathematically, the attention bridge is represented as follows:

Given an original input matrix $\mathbf{H} \in \mathbb{R}^{d_h \times n}$, where $d_h$ is the dimensionality of the input features and $n$ is the number of elements in the input sequence, the attention bridge generates a transformed output matrix $\mathbf{H}^\prime \in \mathbb{R}^{r \times n}$.

Here, $\mathbf{W}_2$ is a weight matrix of dimensions $\mathbb{R}^{r \times d_a}$, where $r$ indicates the number of attention hops. The matrix $\mathbf{W}_1 \in \mathbb{R}^{d_a \times d_h}$ is another weight matrix of dimensions corresponding to the attention dimensions $d_a$. The term "multiple hops" implies the use of multiple attention hops to capture diverse relationships within the data.

The mathematical operations are as follows:

1. Compute the product of the weight matrices $\mathbf{W}_1$ and $\mathbf{H}^\top$:
   $$\mathbf{W}_{1} \mathbf{H}^{\top}$$

2. Apply the hyperbolic tangent (tanh) activation function to the result of the previous step:
   $$\tanh \left(\mathbf{W}_{1} \mathbf{H}^{\top}\right)$$

3. Multiply the outcome of the tanh operation by the weight matrix $\mathbf{W}_2$:
   $$\mathbf{W}_{2} \tanh \left(\mathbf{W}_{1} \mathbf{H}^{\top}\right)$$

4. Apply the softmax function to normalize the weights along the specified dimension:
   $$\operatorname{Softmax}\left(\mathbf{W}_{2} \tanh \left(\mathbf{W}_{1} \mathbf{H}^{\top}\right)\right)$$

5. Finally, perform element-wise multiplication with the original input matrix $\mathbf{H}$:
   $$\operatorname{Softmax}\left(\mathbf{W}_{2} \tanh \left(\mathbf{W}_{1} \mathbf{H}^{\top}\right)\right) \mathbf{H}$$

The resulting matrix $\mathbf{H}^\prime$ encapsulates the enhanced relationships and features that are established through the structured self-attention mechanism. This attention bridge allows the encoded information to be channeled effectively into subsequent language-specific decoding processes, contributing to the overall performance of the architecture.
