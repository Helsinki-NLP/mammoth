...
# Purpose
# -------
# Rank candidate zero-shot evaluation pairs from a Mammoth training
# configuration.
#
# This script reads a `train.yaml`, builds a directed language graph from the
# supervised translation tasks already present in the configuration, computes
# graph-centrality measures for the languages in that graph, and then proposes
# zero-shot source-target pairs that are not directly present as training edges.
#
# Candidate pairs are ranked using a combination of:
...
#   2) extracts directed source-target edges from each task's `src_tgt`,
#   3) builds a language graph over the observed training languages,
#   4) computes language-centrality measures such as degree, closeness,
#      betweenness, PageRank, and eigenvector centrality,
#   5) retrieves URIEL/lang2vec distance matrices where available,
#   6) enumerates candidate zero-shot pairs that:
#        - are not already present as direct training edges,
#        - are reachable within a configurable path-length bound,
#   7) scores each candidate using a blended zero-shot score,
#   8) prints a ranked summary of the proposed zero-shot pairs,
#   9) optionally writes the top-N pairs to an output file.
#
# Inputs
# ------
# Positional:
...
# - Linguistic compatibility currently uses URIEL/lang2vec if available.
# - The script is intended as a planning and selection aid for zero-shot
#   evaluation, not as an inference runner.
...
#    print()
    ...
    #print("Retrieved linguistic distance matrices")
    #if uriel_dmats:
    #    print("  URIEL/l2v:", ", ".join(sorted(uriel_dmats.keys())))
    #else:
    #    print("  URIEL/l2v: (none)")
    if uriel_errors:
