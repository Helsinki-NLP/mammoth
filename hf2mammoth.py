...
def create_weight_mapping(encoder_layers, decoder_layers):
    ...
    mapping["model.encoder.layernorm_embedding.weight"] = (
        "encoder.post_emb_norm.ln.weight"
    )
    ...
    mapping["model.decoder.layernorm_embedding.weight"] = (
        "decoder.net.post_emb_norm.ln.weight"
    )
    mapping["model.decoder.layernorm_embedding.bias"] = (
        "decoder.net.post_emb_norm.ln.bias"
    )
    ...
    for i in range(encoder_layers):
        attn_idx = i * 2
        # Self-attention
        mapping[f"model.encoder.layers.{i}.self_attn_layer_norm.weight"] = (
            f"encoder.attn_layers.layers.{attn_idx}.0.2.ln.weight"
        )
        mapping[f"model.encoder.layers.{i}.self_attn_layer_norm.bias"] = (
            f"encoder.attn_layers.layers.{attn_idx}.0.2.ln.bias"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.q_proj.weight"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_q.weight"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.q_proj.bias"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_q.bias"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.k_proj.weight"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_k.weight"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.k_proj.bias"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_k.bias"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.v_proj.weight"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_v.weight"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.v_proj.bias"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_v.bias"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.out_proj.weight"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_out.weight"
        )
        mapping[f"model.encoder.layers.{i}.self_attn.out_proj.bias"] = (
            f"encoder.attn_layers.layers.{attn_idx}.1.to_out.bias"
        )

        # Feedforward
        ff_idx = attn_idx + 1
        mapping[f"model.encoder.layers.{i}.final_layer_norm.weight"] = (
            f"encoder.attn_layers.layers.{ff_idx}.0.2.ln.weight"
        )
        mapping[f"model.encoder.layers.{i}.final_layer_norm.bias"] = (
            f"encoder.attn_layers.layers.{ff_idx}.0.2.ln.bias"
        )
        mapping[f"model.encoder.layers.{i}.fc1.weight"] = (
            f"encoder.attn_layers.layers.{ff_idx}.1.ff.0.weight"
        )
        mapping[f"model.encoder.layers.{i}.fc1.bias"] = (
            f"encoder.attn_layers.layers.{ff_idx}.1.ff.0.bias"
        )
        mapping[f"model.encoder.layers.{i}.fc2.weight"] = (
            f"encoder.attn_layers.layers.{ff_idx}.1.ff.3.weight"
        )
        mapping[f"model.encoder.layers.{i}.fc2.bias"] = (
            f"encoder.attn_layers.layers.{ff_idx}.1.ff.3.bias"
        )
    ...
    for i in range(decoder_layers):
        ...
        mapping[f"model.decoder.layers.{i}.self_attn_layer_norm.weight"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.0.2.ln.weight"
        )
        mapping[f"model.decoder.layers.{i}.self_attn_layer_norm.bias"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.0.2.ln.bias"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.q_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_q.weight"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.q_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_q.bias"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.k_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_k.weight"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.k_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_k.bias"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.v_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_v.weight"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.v_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_v.bias"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.out_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_out.weight"
        )
        mapping[f"model.decoder.layers.{i}.self_attn.out_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{self_attn_idx}.1.to_out.bias"
        )

        # Cross-attention
        cross_attn_idx = base_idx + 1
        mapping[f"model.decoder.layers.{i}.encoder_attn_layer_norm.weight"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.0.2.ln.weight"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn_layer_norm.bias"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.0.2.ln.bias"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.q_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_q.weight"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.q_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_q.bias"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.k_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_k.weight"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.k_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_k.bias"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.v_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_v.weight"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.v_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_v.bias"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.out_proj.weight"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_out.weight"
        )
        mapping[f"model.decoder.layers.{i}.encoder_attn.out_proj.bias"] = (
            f"decoder.net.attn_layers.layers.{cross_attn_idx}.1.to_out.bias"
        )

        # Feedforward
        ff_idx = base_idx + 2
        mapping[f"model.decoder.layers.{i}.final_layer_norm.weight"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.0.2.ln.weight"
        )
        mapping[f"model.decoder.layers.{i}.final_layer_norm.bias"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.0.2.ln.bias"
        )
        mapping[f"model.decoder.layers.{i}.fc1.weight"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.1.ff.0.weight"
        )
        mapping[f"model.decoder.layers.{i}.fc1.bias"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.1.ff.0.bias"
        )
        mapping[f"model.decoder.layers.{i}.fc2.weight"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.1.ff.3.weight"
        )
        mapping[f"model.decoder.layers.{i}.fc2.bias"] = (
            f"decoder.net.attn_layers.layers.{ff_idx}.1.ff.3.bias"
        )
...
def create_vocabs_dict_from_hf_tokenizer(hf_model_path, src_lang="en", tgt_lang="es"):
    ...
    vocab_items = [
        tok for tok, idx in sorted(tokenizer.get_vocab().items(), key=lambda kv: kv[1])
    ]
    ...
    # Print vocabularies to txt files
    src_vocab_file = f"src_vocab_{src_lang}.txt"
    tgt_vocab_file = f"tgt_vocab_{tgt_lang}.txt"
    ...
    print(f"✓ Saved tgt vocab to: {tgt_vocab_file}")
    return vocabs_dict
...
def create_task_queue_manager(
    src_lang="en", tgt_lang="es", corpus_id="bart_translation", vocabs_dict=None
):
...
def create_xt_to_mammoth_mapping(num_encoder_layers, num_decoder_layers):
    ...
    for layer_idx in range(num_encoder_layers):
        ...
        mapping[f"{xt_attn_base}.1.to_out.weight"] = (
            f"{mammoth_attn_base}.1.to_out.weight"
        )
    ...
    for layer_idx in range(num_decoder_layers):
        ...
        mapping[f"{xt_self_attn_base}.0.2.ln.weight"] = (
            f"{mammoth_self_attn_base}.0.2.ln.weight"
        )
        mapping[f"{xt_self_attn_base}.0.2.ln.bias"] = (
            f"{mammoth_self_attn_base}.0.2.ln.bias"
        )
        mapping[f"{xt_self_attn_base}.1.to_q.weight"] = (
            f"{mammoth_self_attn_base}.1.to_q.weight"
        )
        mapping[f"{xt_self_attn_base}.1.to_q.bias"] = (
            f"{mammoth_self_attn_base}.1.to_q.bias"
        )
        mapping[f"{xt_self_attn_base}.1.to_k.weight"] = (
            f"{mammoth_self_attn_base}.1.to_k.weight"
        )
        mapping[f"{xt_self_attn_base}.1.to_k.bias"] = (
            f"{mammoth_self_attn_base}.1.to_k.bias"
        )
        mapping[f"{xt_self_attn_base}.1.to_v.weight"] = (
            f"{mammoth_self_attn_base}.1.to_v.weight"
        )
        mapping[f"{xt_self_attn_base}.1.to_v.bias"] = (
            f"{mammoth_self_attn_base}.1.to_v.bias"
        )
        mapping[f"{xt_self_attn_base}.1.to_out.weight"] = (
            f"{mammoth_self_attn_base}.1.to_out.weight"
        )
        mapping[f"{xt_self_attn_base}.1.to_out.bias"] = (
            f"{mammoth_self_attn_base}.1.to_out.bias"
        )
        ...
        mapping[f"{xt_cross_attn_base}.0.2.ln.weight"] = (
            f"{mammoth_cross_attn_base}.0.2.ln.weight"
        )
        mapping[f"{xt_cross_attn_base}.0.2.ln.bias"] = (
            f"{mammoth_cross_attn_base}.0.2.ln.bias"
        )
        mapping[f"{xt_cross_attn_base}.1.to_q.weight"] = (
            f"{mammoth_cross_attn_base}.1.to_q.weight"
        )
        mapping[f"{xt_cross_attn_base}.1.to_q.bias"] = (
            f"{mammoth_cross_attn_base}.1.to_q.bias"
        )
        mapping[f"{xt_cross_attn_base}.1.to_k.weight"] = (
            f"{mammoth_cross_attn_base}.1.to_k.weight"
        )
        mapping[f"{xt_cross_attn_base}.1.to_k.bias"] = (
            f"{mammoth_cross_attn_base}.1.to_k.bias"
        )
        mapping[f"{xt_cross_attn_base}.1.to_v.weight"] = (
            f"{mammoth_cross_attn_base}.1.to_v.weight"
        )
        mapping[f"{xt_cross_attn_base}.1.to_v.bias"] = (
            f"{mammoth_cross_attn_base}.1.to_v.bias"
        )
        mapping[f"{xt_cross_attn_base}.1.to_out.weight"] = (
            f"{mammoth_cross_attn_base}.1.to_out.weight"
        )
        mapping[f"{xt_cross_attn_base}.1.to_out.bias"] = (
            f"{mammoth_cross_attn_base}.1.to_out.bias"
        )
    ...
    # Output layers
    mapping["decoder.net.to_logits.weight"] = (
...
def map_xt_to_mammoth_weights(
    xt_model, mammoth_model, num_encoder_layers, num_decoder_layers
):
    ...
    for xt_key, mammoth_key in mapping.items():
        if xt_key not in xt_sd or mammoth_key not in mammoth_sd:
            n_missed += 1
            missed_keys.append(
                f"Missing: XT key '{xt_key}' or Mammoth key '{mammoth_key}'"
            )  # Add to missed_keys
            continue
        ...
        if xt_val.shape != mammoth_val.shape:
            n_missed += 1
            missed_keys.append(
                f"Shape mismatch: XT key '{xt_key}' ({xt_val.shape}) vs Mammoth key '{mammoth_key}' ({mammoth_val.shape})"
            )  # Add to missed_keys
            continue
...
def create_mammoth_model(
    xt_model, hf_model_path, src_lang="en", tgt_lang="es", corpus_id="bart_translation"
):
    ...
    map_xt_to_mammoth_weights(
        xt_model, mammoth_model, config.encoder_layers, config.decoder_layers
    )
...
def convert_hf_to_mammoth(hf_model_path, save_path, src_lang="en", tgt_lang="es"):
    ...
    mammoth_model, model_opts, vocabs_dict, task_queue_manager, optimizer = (
        create_mammoth_model(xt_model, hf_model_path, src_lang, tgt_lang, corpus_id)
    )
...
Examples:
  ...
  python converter.py facebook/bart-base ./models/bart_base --src-lang en --tgt-lang fr
    ...
    parser.add_argument("save_path", help="Path to save the converted Mammoth model")
    ...
    parser.add_argument(
        "--tgt-lang", default="es", help="Target language code (default: es)"
    )
    ...
    if not os.path.exists(args.hf_model_path) and "/" not in args.hf_model_path:
        print(
            f"Warning: {args.hf_model_path} not found locally, trying as HuggingFace model name..."
        )
