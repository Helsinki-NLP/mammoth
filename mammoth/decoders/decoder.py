...
from mammoth.models.stacked_rnn import StackedLSTM, StackedGRU
from mammoth.modules import context_gate_factory, GlobalAttention
from mammoth.utils.rnn_factory import rnn_factory
...
class RNNDecoderBase(DecoderBase):
    ...
    Args:
       ...
       attn_type (str) : see :class:`~mammoth.modules.GlobalAttention`
       attn_func (str) : see :class:`~mammoth.modules.GlobalAttention`
       coverage_attn (str): see :class:`~mammoth.modules.GlobalAttention`
       context_gate (str): see :class:`~mammoth.modules.ContextGate`
       ...
       copy_attn_type (str): The copy attention style. See
        :class:`~mammoth.modules.GlobalAttention`.
    """
    ...
    def __init__(
        ...
        self.rnn = self._build_rnn(
            rnn_type, input_size=self._input_size, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout
        )
        ...
        if not self.attentional:
            ...
            self.attn = GlobalAttention(hidden_size, coverage=coverage_attn, attn_type=attn_type, attn_func=attn_func)

        if copy_attn and not reuse_copy_attn:
            if copy_attn_type == "none" or copy_attn_type is None:
                raise ValueError("Cannot use copy_attn with copy_attn_type none")
            self.copy_attn = GlobalAttention(hidden_size, attn_type=copy_attn_type, attn_func=attn_func)
        else:
            self.copy_attn = None
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    See :class:`~mammoth.decoders.decoder.RNNDecoderBase` for options.
