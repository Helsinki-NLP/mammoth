import unittest
from mammoth.modules.embeddings import Embeddings
        ...
        for params, init_case in itertools.product(self.PARAMS, self.cases()):
            ...
            for key in emb.state_dict():
                if key not in trainable_params:
                    if key.endswith("emb_luts.0.weight") and init_case["freeze_word_vecs"]:
            ...
            if init_case["freeze_word_vecs"]:
                self.assertFalse(
                    any(trainable_param.endswith("emb_luts.0.weight") for trainable_param in trainable_params),
