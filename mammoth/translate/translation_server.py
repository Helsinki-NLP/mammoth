    ...
    def wrapper(server_model, *args, **kwargs):
        if sys.version_info[0] == 3:
            if not server_model.running_lock.acquire(True, 120):
                raise ServerModelError("Model %d running lock timeout" % server_model.model_id)
        else:
...
class CTranslate2Translator(object):
    ...
    reproduce the mammoth.translate.translator API.
    """

    def __init__(self, model_path, ct2_translator_args, ct2_translate_batch_args, target_prefix=False, preload=False):
        import ctranslate2
        ...
        if preload:
            ...
            print("Performed a dummy translation to initialize the model", dummy_translation)
            time.sleep(1)
...
class ServerModel(object):
    ...
    def __init__(
        ...
        self.ct2_model = os.path.join(model_root, ct2_model) if ct2_model is not None else None
    ...
    def run(self, inputs):
        ...
        if not self.loading_lock.is_set():
            self.logger.info("Model #%d is being loaded by another thread, waiting" % self.model_id)
        ...
        results = [self.maybe_detokenize_with_align(result, src) for result, src in zip(results, tiled_texts)]
