...
class LossComputeBase(nn.Module):
    ...
    def __call__(self, batch, output, attns, normalization=1.0, shard_size=0, trunc_start=0, trunc_size=None):
        ...
        batch_stats = mammoth.utils.Statistics()
    ...
    def _stats(self, loss, scores, labels):
        ...
        Returns:
            :obj:`mammoth.utils.Statistics` : statistics for this batch.
        """
        pred = scores.max(1)[1]
        non_padding = labels.ne(self.padding_idx)
        num_correct = pred.eq(labels).masked_select(non_padding).sum().item()
        num_non_padding = non_padding.sum().item()
        return mammoth.utils.Statistics(loss.item(), num_non_padding, num_correct)
...
class CommonLossCompute(LossComputeBase):
    ...
    def _add_align_shard_state(self, shard_state, batch, range_start, range_end, attns):
        ...
        ref_align = mammoth.utils.make_batch_align_matrix(align_idx, align_matrix_size, normalize=True)
