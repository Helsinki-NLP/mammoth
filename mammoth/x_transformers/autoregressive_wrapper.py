...
class AutoregressiveWrapper(Module):
    ...
    def generate(
        ...
        **kwargs
        ...
        for _ in range(seq_len):
            ...
            if not exists(eos_token):
                continue
            # Always compute is_eos_tokens when eos_token exists
            is_eos_tokens = (out == eos_token)
            if bart_style_eos:
                # Simple logic: first token is start EOS, check for second EOS
                if out.shape[1] > 1:  # We have more than just the start EOS
                    last_tokens = out[:, -1]  # Get the last token for each batch
                    if (last_tokens == eos_token).all():  # All batches generated EOS
                        break
            else:
                # Original logic
                if is_eos_tokens.any(dim = -1).all():
        ...
        if exists(eos_token):
            if bart_style_eos:
                # For BART-style: only mask after the SECOND EOS token
                # First EOS is start token, second EOS is actual end token
                eos_cumsum = is_eos_tokens.float().cumsum(dim=-1)
                
                # Create mask for positions after second EOS
                shifted_is_eos_tokens = F.pad(is_eos_tokens, (1, -1))
                shifted_eos_cumsum = F.pad(eos_cumsum, (1, -1), value=0)
                
                # Only mask positions where we've seen 2+ EOS tokens
                mask = (shifted_eos_cumsum >= 2)
                out = out.masked_fill(mask, self.pad_value)
            else:
                # Original logic: mask after first EOS
                shifted_is_eos_tokens = F.pad(is_eos_tokens, (1, -1))
                mask = shifted_is_eos_tokens.float().cumsum(dim = -1) >= 1
                out = out.masked_fill(mask, self.pad_value)
    ...
    def forward(self, x, return_outputs = False, **kwargs):
        seq, ignore_index, add_attn_z_loss, add_next_embed_loss= x.shape[1], self.ignore_index, self.add_attn_z_loss,self.add_next_embed_loss
