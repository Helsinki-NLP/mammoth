...
from mammoth.x_transformers.attend import Attend, Intermediates
from mammoth.x_transformers.autoregressive_wrapper import AutoregressiveWrapper
...
def detach_all(obj):
    return tree_map(
        lambda t: t.detach() if is_tensor(t) and t.requires_grad else t, obj
    )
...
def calc_entropy(t: Tensor, is_prob=False):
...
class AbsolutePositionalEmbedding(Module):
    def __init__(self, dim, max_seq_len, scale: float, l2norm_embed=False):
        super().__init__()
        if scale is None:  # by default
            # self.scale = dim ** -0.5 if not l2norm_embed else 1.
            self.scale = 1.0  # Bart specific
        else:
            self.scale = scale
    ...
    def forward(self, x, pos=None, seq_start_pos=None, offset=0):
        ...
        # pos = (pos - 2).clamp(min = 0)

        pos_emb = self.emb(pos)
        pos_emb = pos_emb * self.scale
        return l2norm(pos_emb) if self.l2norm_embed else pos_emb

        # pos_emb = l2norm(pos_emb) if self.l2norm_embed else pos_emb
        # # Expand to match batch dimension: (seq_len, dim) -> (batch_size, seq_len, dim)
...
class LayerNorm(Module):
    def __init__(
        ...
        if bias:
            # Use PyTorch's standard LayerNorm when bias is needed
            self.ln = nn.LayerNorm(dim, bias=True)
        else:
            # Use the original bias-less approach
    ...
    def forward(self, x):
        if self.bias:
            # Let PyTorch's LayerNorm handle everything
            return self.ln(x)
        else:
            normed = self.ln(x)
...
class DynamicLIMe(Module):
    ...
    def forward(self, x, hiddens):
        ...
        assert (
            hiddens.shape[0] == self.num_layers
...
class FeedForward(Module):
    def __init__(
        ...
        self.ff = Sequential(
            ...
            nn.Dropout(dropout) if sublayer_dropout > 0.0 else None,
        )
    ...
    def forward(self, x, deep_embed=None):
        out = self.ff(x)
...
class Attention(Module):
    def __init__(
        ...
        onnxable=False,
        qkv_bias=True,  # Enable bias for Q, K, V projections
        out_bias=True,  # Enable bias for output projection
        ...
        self.to_q = nn.Linear(dim, q_dim, bias=qkv_bias)
        self.to_k = nn.Linear(dim_kv, k_dim, bias=qkv_bias)
        self.to_v = nn.Linear(dim_kv, v_dim, bias=qkv_bias)
        ...
        dim_out = default(dim_out, dim)
        self.to_out = (
            nn.Sequential(nn.Linear(out_dim, dim * 2, bias=out_bias), nn.GLU())
            if on_attn
            else nn.Linear(out_dim, dim, bias=out_bias)
        )
        ...
        assert not (
            is_partial_rotate_heads and kv_heads < heads
    ...
    def forward(
        ...
        assert not (
            qkv_receive_diff_residuals and has_context
        ...
        if exists(additional_key_values):
            ...
            if added_kv_heads != kv_h:
                ...
                k, v, added_k, added_v = tuple(
                    repeat(t, "b h ... -> b (r h) ...", r=h // t.shape[1])
                    for t in (k, v, added_k, added_v)
                )
            ...
            if exists(input_mask) or exists(additional_key_value_mask):
                ...
                if not exists(additional_key_value_mask):
                    input_mask = pad_at_dim(
                        input_mask, (added_kv_len, 0), dim=-1, value=True
                    )
                elif not exists(input_mask):
                    input_mask = pad_at_dim(
                        additional_key_value_mask, (0, seq_len), dim=-1, value=True
                    )
                else:
...
class AttentionLayers(Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Extract layernorm_bias and post_emb_norm before the assertion
        layernorm_bias = kwargs.pop("layernorm_bias", False)
        ...
        assert at_most_one_of(
            alibi_pos_bias, rel_pos_bias, data_dependent_alibi
        ...
        assert at_most_one_of(
            rel_pos_bias, dynamic_pos_bias, alibi_pos_bias
        ...
        # Modify norm_fn creation:
        norm_fn = partial(norm_class, dim, bias=layernorm_bias)
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(
            ...
            post_main_norm = LayerNorm(dim, bias=True) if not pre_norm else None
    ...
    def forward(
        ...
        self_attn_additional_kv: (
            LayerIntermediates | list[tuple[Tensor, Tensor]] | None
        ) = None,
        ...
        if exists(self_attn_additional_kv) and route_additional_kv_to_top:
            ...
            num_self_attns = sum(
                [layer_type == "a" for layer_type in first(layer_variables)]
            )

            self_attn_additional_kv = self_attn_additional_kv[-num_self_attns:]
            self_attn_additional_kv = [None] * (
                num_self_attns - len(self_attn_additional_kv)
            ) + self_attn_additional_kv
        ...
        for ind, (
            ...
            if layer_type == "a":
                out, inter = block(
                    ...
                    pos=pos,
                    rotary_pos_emb=rotary_pos_emb,
                    additional_key_values=next(iter_self_attn_kv, None),
                    additional_key_value_mask=additional_kv_mask,
                    prev_attn=prev_attn,
...
class TransformerWrapper(Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.l2norm_embed = l2norm_embed
        final_logits_bias = kwargs.get("use_final_logits_bias", True)
        # if final_logits_bias:
        #     logits_dim = default(logits_dim, num_tokens)
        #     self.final_logits_bias = nn.Parameter(torch.zeros(1, logits_dim))
        ...
        if no_abs_pos_emb:
            ...
            self.pos_emb = AbsolutePositionalEmbedding(
                emb_dim, max_seq_len, scale=1.0, l2norm_embed=l2norm_embed
            )
        ...
        self.post_emb_norm = (
            LayerNorm(emb_dim, bias=post_emb_norm_bias)
            if post_emb_norm
            else nn.Identity()
        )
        ...
        if return_only_embed:
            ...
            self.to_logits = nn.Linear(dim, logits_dim, bias=final_logits_bias)
    ...
    def forward(
        ...
        **kwargs,
        ...
        seq_pos_offset = kwargs.pop("seq_pos_offset", 2)
        ...
        assert not (
            exists(self.embeds) ^ (len(embed_ids) > 0)
        ...
        x = self.post_emb_norm(x)
        # print(f"tensor shape after post_emb_norm: {x.shape}")
        # print(f"input after post_emb_norm: {x}")
        ...
        if not self.recycling:
            assert (
                not exists(recycle_steps) or recycle_steps == 1
            ...
            # intermediates is a list, its only element is the object Layerintermediates,
            # which is what we want
            # Only execute when mammoth model is activated (has attention_layers_stack)
            ...
            assert (
                exists(recycle_steps) and recycle_steps > 0
        ...
        # Handle both MAMMOTH's modified structure and standard x-transformers
        if hasattr(self.attn_layers, "attention_layers_stack"):
            is_encoder = not self.attn_layers.attention_layers_stack[0].causal
        else:
            # For standard x-transformers Encoder/Decoder classes
            is_encoder = not getattr(self.attn_layers, "causal", False)
        return_pooled_tokens = exists(self.attn_pool) and is_encoder

        if exists(self.attn_pool) and (
        ...
        if exists(combine_mixture):
            with autocast("cuda", enabled=False):
                ...
                logits = log(mos)
            # Add final logits bias if it exists
            if exists(self.final_logits_bias):
                if isinstance(logits, tuple):
                    logits = tuple(l + self.final_logits_bias for l in logits)
                else:
...
class XTransformer(Module):
    def __init__(
        ...
        **kwargs,
        ...
        enc_kwargs["layernorm_bias"] = layernorm_bias
        dec_kwargs["layernorm_bias"] = layernorm_bias
        assert (
        ...
        enc_transformer_kwargs["post_emb_norm_bias"] = enc_kwargs.pop(
            "post_emb_norm_bias", True
        )

        dec_transformer_kwargs = pick_and_pop(
            ["num_tokens", "max_seq_len", "use_final_logits_bias"], dec_kwargs
        )
        ...
        dec_transformer_kwargs["post_emb_norm"] = dec_kwargs.pop("post_emb_norm", False)
        dec_transformer_kwargs["post_emb_norm_bias"] = dec_kwargs.pop(
            "post_emb_norm_bias", True
        )
