...
class GGNNPropogator(nn.Module):
    ...
    def __init__(self, state_dim, n_node, n_edge_types):
        ...
        self.tansform = nn.Sequential(nn.Linear(state_dim * 3, state_dim), nn.LeakyReLU())
...
class GGNNEncoder(EncoderBase):
    ...
    def __init__(
        ...
        assert found_n_minus_one, "GGNN src_vocab must include node numbers for edge connections"
        ...
        # Token embedding
        if src_ggnn_size > 0:
            self.embed = nn.Sequential(nn.Linear(src_ggnn_size, src_word_vec_size), nn.LeakyReLU())
            assert self.src_ggnn_size >= self.DELIMITER, "Embedding input must be larger than vocabulary"
            assert self.src_word_vec_size < self.state_dim, "Embedding size must be smaller than state_dim"
        else:
            assert self.DELIMITER < self.state_dim, "Vocabulary too large, consider -src_ggnn_size"
    ...
    def forward(self, src, lengths=None):
        ...
        token_onehot = np.zeros(
            (batch_size, nodes, self.src_ggnn_size if self.src_ggnn_size > 0 else self.state_dim), dtype=np.int32
        )
        edges = np.zeros((batch_size, nodes, nodes * self.n_edge_types * 2), dtype=np.int32)
        ...
        for i in range(batch_size):
            ...
            for j in range(len(npsrc)):
                ...
                if not tokens_done:
                    ...
                    edge += 1
                    assert source_node == -1, f'Error in graph edge input: {source_node} unpaired'
                    ...
                    if source_node < 0:
                        ...
                        if self.bidir_edges:
                            edges[i][num][nodes * (edge + self.n_edge_types) + source_node] = 1
                        source_node = -1
        ...
        if self.src_ggnn_size > 0:
            ...
            prop_state = torch.cat(
                (
                    token_embed,
                    torch.zeros((batch_size, nodes, self.state_dim - self.src_word_vec_size)).float().to(src.device),
                ),
                2,
            )
        else:
        ...
        for i_step in range(self.n_steps):
            ...
            prop_state = self.propogator(in_states, out_states, prop_state, edges, nodes)
