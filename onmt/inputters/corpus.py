...
class DatasetAdapter(object):
    ...
    valid_field_name = ('src', 'tgt', 'indices', 'src_map', 'src_ex_vocab', 'alignment', 'align')
    ...
    def _process(item, is_train):
        ...
        maybe_example = transform.apply(example.copy(), is_train=is_train, corpus_name=cid)
    ...
    def _maybe_add_dynamic_dict(self, example, fields):
        ...
        if 'src_map' in fields and 'alignment' in fields:
            example = _dynamic_dict(example, fields['src'].base_field, fields['tgt'].base_field)
        return example
...
class ParallelCorpus(object):
        ...
        with exfile_open(self.src, mode='rb') as fs, exfile_open(self.tgt, mode='rb') as ft, exfile_open(
            ...
            for i, (sline, tline, align, *features) in enumerate(zip(fs, ft, fa, *features_files)):
                if (i % stride) == offset:
                    ...
                    # transforms.
                    example = {'src': sline, 'tgt': tline, 'src_original': sline, 'tgt_original': tline}
    ...
    def __str__(self):
        ...
        return '{}({}, {}, align={}, src_feats={})'.format(cls_name, self.src, self.tgt, self.align, self.src_feats)
...
class ParallelCorpusIterator(object):
    ...
    def _add_index(self, stream):
        for i, item in enumerate(stream):
            ...
            if len(example['src']) == 0 or len(example['tgt']) == 0 or ('align' in example and example['align'] == 0):
                # empty example: skip
...
def build_corpora_iter(
    corpus_id, corpus, transforms, corpus_info, skip_empty_level='warning', stride=1, offset=0
    ...
    corpus_iter = ParallelCorpusIterator(
        corpus, transform_pipe, skip_empty_level=skip_empty_level, stride=stride, offset=offset
    )
    return corpus_iter
...
def build_sub_vocab(corpus_id, corpus, transforms, opts, n_sample, stride, offset):
    ...
    for i, item in enumerate(c_iter):
        ...
        if opts.dump_samples:
            build_sub_vocab.queues[corpus_id][offset].put((i, src_line_pretty, tgt_line))
...
def build_vocab(opts, corpus_id, transforms, n_sample=3):
    ...
    with mp.Pool(opts.num_threads, init_pool, [queues]) as p:
        func = partial(build_sub_vocab, corpus_id, corpora[corpus_id], transforms, opts, n_sample, opts.num_threads)
        for sub_counter_src, sub_counter_tgt, sub_counter_src_feats in p.imap(func, range(0, opts.num_threads)):
...
def save_transformed_sample(opts, corpus_id, transforms, n_sample=3):
    ...
    c_iter = build_corpora_iter(
        corpus_id, corpus, transforms, opts.data[corpus_id], skip_empty_level=opts.skip_empty_level
    )
    sample_path = os.path.join(os.path.dirname(opts.save_data), CorpusName.SAMPLE)
    os.makedirs(sample_path, exist_ok=True)
    dest_base = os.path.join(sample_path, "{}.{}".format(corpus_id, CorpusName.SAMPLE))
    with open(dest_base + ".src", 'w', encoding="utf-8") as f_src, open(
        dest_base + ".tgt", 'w', encoding="utf-8"
    ) as f_tgt:
        for i, item in enumerate(c_iter):
